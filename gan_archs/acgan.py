...
class Generator(nn.Module):
    def __init__(self, img_shape, latent_dim, nb_classes):
        ...
        self.label_emb = nn.Embedding(self.NB_CLASSES, self.LATENT_DIM)
...
class Discriminator(nn.Module):
    def __init__(self, img_shape, nb_classes):
        ...
        # Output layers
        #TODO: DOES NOT OUTPUT LOGITS!!!
        self.adv_layer = nn.Sequential(nn.Linear(128 * ds_size ** 2, 1),
                                        nn.Sigmoid()
                                        )
        self.aux_layer = nn.Sequential(nn.Linear(128 * ds_size ** 2, self.NB_CLASSES),
                                        nn.Softmax(dim=1)
                                        )
